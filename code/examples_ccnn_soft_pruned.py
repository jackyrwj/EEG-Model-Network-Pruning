...
if __name__ == "__main__":
    ...
    file_handler = logging.FileHandler(
        './tmp_out/examples_ccnn/examples_ccnn_fine-tune_sft70.log')
    ...
    console_handler = logging.StreamHandler()
    file_handler = logging.FileHandler(
        './tmp_out/examples_ccnn_seed/examples_ccnn_fine-tuned_sft70.log')
    logger.addHandler(console_handler)
    logger.addHandler(file_handler)
    dataset = SEEDDataset(
        ...
        # if 5 <= i <= 9:
            ...
            for t in range(epochs):
                ...
                if (t % 1 == 0 or t == epochs-1):
                    ...
                    for key in range(0, 7, 2):
                        compress_rate[key] = percent
                    # mask_index(0, 3, 6, 9....)
                    mask_index = [x for x in range(0, 7, 2)]
                    ...
                    writer.add_scalars(
                        f"ccnn/sft70/model{i}", val_acc, t)
                    if val_acc > best_val_acc:
                        best_val_acc = val_acc
                        torch.save(model.state_dict(), 
                        # f'./tmp_out/examples_ccnn/fine-tuned_model/sft70/fine-tuned_model{i}.pt')
            #     t4 = time.time()
            #     print(t4 - t3)
