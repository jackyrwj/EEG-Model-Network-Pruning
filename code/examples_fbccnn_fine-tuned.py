...
if __name__ == "__main__":
    ...
    #     # './tmp_out/examples_fbccnn/examples_fbccnn_fine-tuned_weight30.log')
    ...
    console_handler = logging.StreamHandler()
    file_handler = logging.FileHandler(
        # './tmp_out/examples_fbccnn_seed/examples_fbccnn_fine-tuned_weight30.log')


        './tmp_out/examples_fbccnn_seed/tmp.log')
    ...
    k_fold = KFold(
        n_splits=10, split_path=f'./tmp_out/examples_fbccnn_seed/split', shuffle=True)
    # -----------------------------------------------
    ...
    for i, (train_dataset, test_dataset) in enumerate(k_fold.split(dataset)):
        if i < 10:
            ...
            checkpoint = torch.load(
                ...
                f'./tmp_out/examples_fbccnn_seed/tmp/model{i}.pt')

            # weight
            # cfg = [12, 32, 64, 128, 256, 128, 32]
            ...
            #     # writer.add_scalars(
            #         # f"fbccnn/fine-tuned/weight30/loss", {"Train":train_loss}, t)

            #     val_acc, val_loss=valid(val_loader, model, loss_fn)
            #     # writer.add_scalars(
            #         # f"fbccnn/fine-tuned/weight30/loss", {"Val":val_loss}, t)

            #     if val_acc > best_val_acc:
            #         best_val_acc = val_acc
            #         torch.save(model.state_dict(), 
            #         f'./tmp_out/examples_fbccnn/fine-tuned_model/weight30/fine-tuned_model{i}.pt')
            #     t4 = time.time()
            #     print(t4 - t3)
