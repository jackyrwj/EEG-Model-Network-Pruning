...
def print_model_param_nums(model=None):
    ...
    print('  + Number of params: %fM' % (total / 1e6))
    return total


# def count_model_param_flops(model=None, input_res=224, multiply_adds=True):
def count_model_param_flops(model=None, channel = 4, x = 9, y = 9, multiply_adds=True):
    ...
    #     model = torchvision.models.alexnet()
    foo(model)
    # input = Variable(torch.rand(4,input_res,input_res).unsqueeze(0), requires_grad = True).cuda()
    input = Variable(torch.rand(channel, x, y).unsqueeze(0), requires_grad = True).cuda()
    out = model(input)
    total_flops = (sum(list_conv) + sum(list_linear) + sum(list_bn) + sum(list_relu) + sum(list_pooling) + sum(list_upsample))

    print('  + Number of FLOPs: %.2fG' % (total_flops / 1e9))
    # print('  + Number of FLOPs: %fG' % (total_flops / 1e9))
    return total_flops